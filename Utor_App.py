...
# --- PROFIT ANALYSIS PAGE ---
    ...
    try:
        ...
        if not lucro_data:
            ...
            with col2:
                distributor_list = sorted(df_lucro["Distribuidor"].dropna().unique())
                selected_distributors = st.multiselect(
                    "Select Distributors",
                    distributor_list,
                    default=distributor_list
                )
            
            # Filter data
            df_lucro_filtered = df_lucro[
                (df_lucro["Chave"].isin(selected_chaves)) &
                (df_lucro["Distribuidor"].isin(selected_distributors))
            ]
            ...
            avg_profit = lucro_summary["Lucro_Empresa"].mean()
            
            # KPIs
            st.markdown("### 🎯 Profit KPIs")
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                st.metric("Total Profit", format_currency(total_sum))
            with col2:
                st.metric("Average Profit/Period", format_currency(avg_profit))
            with col3:
                st.metric("Active Distributors", len(selected_distributors))
            with col4:
                st.metric("Time Periods", len(selected_chaves))
